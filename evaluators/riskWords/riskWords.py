...
def process_folder(folder_path, risk_words):
    total_ratios = 0
    total_files = 0
    ...
    for filepath in glob.glob(os.path.join(folder_path, '*.txt')):
        total_files += 1
        ratio_of_risk_words = get_ratio_of_risk_words(filepath, count_risk_words(tokenize(filepath), risk_words))
        #print(total_files)
...
def evaluate(file_path, risk_words, type_of_risk_words):
    ratio = 0
    shifted1 = 0
    shifted2 = 0
    
    if type_of_risk_words == LOADED_LANGUAGE:
        ratio = get_ratio_of_risk_words(file_path, count_risk_words(tokenize(file_path), risk_words))
...
for filepath in glob.glob(os.path.join(folder_path, '*.txt')):
    total_files += 1
    eval_score = evaluate(filepath, emotional_language, EMOTIONAL_LANGUAGE)
    #print(total_files)
