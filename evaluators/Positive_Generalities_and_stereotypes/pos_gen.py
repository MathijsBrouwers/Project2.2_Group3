...
def detect_positive_generalities(tokens):
...
def calculate_propaganda_score(tokens, detected_generalities):
    ...
    if(total_words) > 0 :
        ...
        propaganda_score = 0
    return function(propaganda_score)

def run(file_path):
    tokens = tokenize(file_path)
    generalties = detect_positive_generalities(tokens)
    score = calculate_propaganda_score(tokens, generalties)
    return score
